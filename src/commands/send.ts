...
export const sendEmailCommand = new SlashCommandBuilder()
  ...
  .addAttachmentOption((option) =>
    option
      .setName('sheet')
      .setDescription('📂 Please upload an Excel file in .xlsx or .csv extension')
      .setRequired(true)
  );

export async function sendEmailViaExcel(interaction: ChatInputCommandInteraction) {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  const attachment = interaction.options.getAttachment('sheet') as Attachment;

  if (!attachment || !(attachment.name.endsWith('.xlsx') || attachment.name.endsWith('.csv'))) {
    return interaction.editReply('⚠️ Please upload a valid Excel file (.xlsx) or (.csv)');
  }
