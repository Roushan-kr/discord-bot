...
export const handleExcelUpload = async (
  ...
  try {
    ...
    if (filteredSheet.length === 0) {
      return interaction.editReply(
        '⚠️ No valid rows found. Ensure all fields are filled.'
      );
    }
    ...
    if (
      emails.length !== messages.length ||
      messages.length !== subjects.length
    ) {
      return interaction.editReply(
        '⚠️ All rows must have values for `sandTo`, `subject`, and `message`.'
      );
    }
